...
function changeDarkMode() {
    ...
    // 색상 설정
    const color1 = isDarkModeEnabled ? '#00B4ED' : '#000';
    const color2 = isDarkModeEnabled ? '#5CD2E6' : '#393939';
...
document.getElementById("login-btn").addEventListener('click', async function (e) {
    ...
    if (!memberId || !memberPassword) {
        showReusableModal("아이디와 비밀번호를 모두 입력해주세요.");
        return;
    }
    ...
    try {
        ...
        if (response.ok) {
            ...
            // UI 업데이트
            updateUIAfterLogin(result.memberType);
        ...
        showReusableModal("로그인 중 오류가 발생했습니다. 다시 시도해주세요.");
        loginModalContent.classList.add("shaking");
        setTimeout(() => {
            loginModalContent.classList.remove('shaking');
        }, 500);
...
// 로그인 후 UI 업데이트
function updateUIAfterLogin(memberType) {
